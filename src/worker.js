/**
 * Cloudflare Worker with Static Assets Support for Nakshatra Designer's
 * Serves static frontend assets (dist/) with single-page-application routing,
 * and handles all /api/* requests with zero-latency edge JSON responses.
 */
...
export default {
  async fetch(request, env, ctx) {
    ...
    // API Routes handled by Worker logic
    if (url.pathname.startsWith('/api')) {
      return handleApi(request, env);
    }

    // Static assets handled by Cloudflare Assets binding
    if (env.ASSETS) {
